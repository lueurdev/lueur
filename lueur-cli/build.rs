...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    build_package("lueur-ebpf-programs");
...
fn build_package(package_name: &str) {
    let Metadata { packages, .. } =
        MetadataCommand::new().no_deps().exec().unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `lueur-ebpf` and
        // `lueur-ebpf`, so cargo will automatically rebuild us
        // if `lueur-ebpf-*`'s *library* target or any of its dependencies
        // change. Since we depend on `lueur-ebpf-`'s *binary* targets,
        // that only gets us half of the way. This stanza ensures cargo
        ...
        let ebpf_target_dir = out_dir.join(package_name);
            ...
            match message.expect("valid JSON") {
                Message::CompilerArtifact(Artifact {
                    ...
                    if let Some(executable) = executable {
                        executables
                            .push((name, executable.into_std_path_buf()));
                    }
                }
        ...
        for (name, binary) in executables {
            ...
            let _: u64 = fs::copy(&binary, &dst).unwrap_or_else(|err| {
                panic!("failed to copy {binary:?} to {dst:?}: {err}")
            });
        }
    } else {
        ...
        for Target { name, kind, .. } in targets {
            ...
            let dst = out_dir.join(name);
            fs::write(&dst, []).unwrap_or_else(|err| {
                panic!("failed to create {dst:?}: {err}")
            });
        }
    }
}
